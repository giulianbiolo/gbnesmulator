...
impl<'a> Bus<'a> {
    pub fn new<'call, F>(rom: Rom, gameloop_callback: F) -> Bus<'call> where F: FnMut(&NesPPU, &mut Joypad) + 'call {
        let ppu: NesPPU = NesPPU::new(rom.chr_rom, rom.screen_mirroring);
        Bus { cpu_vram: [0; 2048], prg_rom: rom.prg_rom, ppu, cycles: 0, gameloop_callback: Box::new(gameloop_callback), joypad1: Joypad::new() }
    }
    ...
    pub fn tick(&mut self, cycles: u8) {
        ...
        if new_frame { (self.gameloop_callback)(&self.ppu, &mut self.joypad1); }
    }
...
impl Mem for Bus<'_> {
    ...
    fn mem_write(&mut self, addr: u16, data: u8) {
        match addr {
            ...
            0x4000..=0x4015 => {
                // APU and I/O registers
                println!("Write to APU at {:2X} Data: {:X}", addr, data);
            },
