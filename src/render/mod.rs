...
use crate::{ppu::{NesPPU, registers::control::FlagArithmetic}, cartridge::Mirroring};
use frame::Frame;

fn bg_pallette_old(ppu: &NesPPU, tile_column: usize, tile_row: usize) -> [u8; 4] {
...
/*
pub fn render_old(ppu: &NesPPU, frame: &mut Frame) {
    let bank = ppu.bknd_pattern_addr();
    ...
    for i in (0..ppu.oam_data.len()).step_by(4).rev() {
        ...
        let tile: &[u8] = &ppu.chr_rom[(bank + tile_idx * 16) as usize..=(bank + tile_idx * 16 + 15) as usize];
